...
<div class="wrapper" id="wrapper">
            ...
            <div class="container-wrapper">
                ...
                <article>
                    <div class="contents" id="contents">
                        ...
                        <?
                        $arr_POSTVAL = array("resume_num", "wr_name", "wr_hp1", "wr_hp2", "wr_hp3", "wr_pass");
						
                        foreach ( $_POST as $k => $v ) {
                            if ( in_array($k, $arr_POSTVAL) > 0 && ( trim($v) == "" || !$v ) ) {
                               // echo "<script> alert('정상적인 방법으로 이용하세요.'); location.replace('/');  </script>";
                            } else {
                                // Mysql Encoding : Euc-kr
                                ${$k."_db"} = iconv("utf-8", "cp949", $v);
                                ${$k."_db"} = $v;
                            }
                        }

                        $phone = $wr_hp1."-".$wr_hp2."-".$wr_hp3;
                        $enc_pass = base64_encode($wr_pass);			// 비밀번호 암호화

                        // 새로운 입력인지 수정인지 구분
                        $sql = " select * from employment where kor_name = '".$wr_name."' and phone = '".$phone."' and resume_num = '".$resume_num."' ";
                        $rs = mysql_query($sql);
                        if ( mysql_num_rows($rs) > 0 ) {
							...
							if($row['password'] != $enc_pass){
								echo "<script>alert('비밀번호를 확인하세요.'); history.back()</script>";exit;
							}
                            $url = "resume.php?j=u&wr_id=".$row[wr_id]."&pass=".$enc_pass;
                        ...
                        <div class="cont">
                            <form name="frm" method="POST" action="./resume.php" onsubmit="return check_form(this)">
                                <input type="hidden" name="kor_name" value="<?=$wr_name?>" />
    ...
    <script>
        function check_form(f){
            ...
            if(chk==0 || agree[1].checked==true){alert("개인정보제공에 동의하셔야 이용가능합니다.");return false;}
        }
    </script>
