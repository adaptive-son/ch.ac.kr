...
if($file1_size>0&&$file1) {
    ...
    if($file1_size>0) {
        ...
        if(file_exists("./file_data/$id/".$temp1_name)) {
            ...
            $file_name1 = "/recruit3/file_data/$id/".$reg_date."/".$temp1_name;
            ...
            $file_name1 = "/recruit3/file_data/$id/".$temp1_name;
            @chmod($file_name1,0777);
        }
    }
}
...
if($j==""){
    ...
    $resume_data = mysql_fetch_array(mysql_query("SELECT apply_num FROM recruit_te_new WHERE wr_id='$wr_id'"));
...
if($j==""){
    $sql = "INSERT recruit_te_copy SET apply_num='$apply_num',$query1 ,file_name='$file_name1', s_file_name='$s_file_name1'";
    $result = mysql_query($sql);
    $wr_ins_id = mysql_insert_id();
    $sql1 = "INSERT recruit_te SET parent = '$wr_ins_id',wr_datetime=now(),type_gubun='$type_gubun',$query2";
    $result1 = mysql_query($sql1);
    if(!$result1) error_log("recruit3/proc.php INSERT recruit_te 실패 (wr_id=$wr_ins_id): ".mysql_error());
    $sql2 = "INSERT recruit_te_school_addr SET parent = '$wr_ins_id',$query3";
    $result2 = mysql_query($sql2);
    if(!$result2) error_log("recruit3/proc.php INSERT recruit_te_school_addr 실패 (wr_id=$wr_ins_id): ".mysql_error());
    ...
    $sql = "UPDATE recruit_te_copy SET apply_num='$apply_num', $query1 ";

    $result = mysql_query($sql);
    $sql1 = "UPDATE recruit_te SET type_gubun='$type_gubun',$query2 WHERE parent='$wr_id'";
    $result1 = mysql_query($sql1);
    if(!$result1) error_log("recruit3/proc.php UPDATE recruit_te 실패 (wr_id=$wr_id): ".mysql_error());
    $sql2 = "UPDATE recruit_te_school_addr SET $query3 WHERE parent='$wr_id'";
    //print_R($sql2);exit;
    $result2 = mysql_query($sql2);
    if(!$result2) error_log("recruit3/proc.php UPDATE recruit_te_school_addr 실패 (wr_id=$wr_id): ".mysql_error());
    if($result && $result1 && $result2){
