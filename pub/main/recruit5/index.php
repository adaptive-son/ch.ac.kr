            ...
            <div class="container-wrapper">
                ...
                <article>
                    <div class="contents" id="contents">
                        ...
                        if ( strtotime(date("Y-m-d H:i:s")) <= strtotime(date("2017-02-28 18:00:00")) ) {
                            // 임시 오픈 ( 비공개 추가 접수를 위함 )
                            $row[idx] = '7';
                        } else {
                            ...
                            //채용중인 공고가 있는지 체크한다.
                            $today = date("Y-m-d");
                            $sql = "SELECT idx FROM recruit_index_research WHERE date(start_date) <= date(now()) AND date(end_date) >= date(now())";
                            $res = mysql_query($sql);
                            ...
                            if (!$__IS_ADB) {
                                if (!$row || count($row) == 0) {
                                    ...
                                    // 시작시간
                                    $ing_dateTime = strtotime(date("2017-02-01 15:00:00"));
                                    /*
                                    if ( strtotime(date("Y-m-d H:i:s")) <= $ing_dateTime ) {
                                        echo "<script> alert('진행중인 채용공고가 없습니다.'); history.back(); </script>";
                                        exit;
                                    }
                                    */
