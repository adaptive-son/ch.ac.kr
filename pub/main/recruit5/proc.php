...
if ( $j == "" ) {
    $dup_check_sql = " select wr_id from recruit_copy_research where kor_name = '".addslashes($kor_name)."' and phone = '".addslashes($phone)."' and resume_num = '".addslashes($resume_num)."' ";
...
$apply_count=mysql_num_rows(mysql_query("SELECT * FROM recruit_copy_research WHERE resume_num='$resume_num'"));
...
if($j==""){
    ...
    $resume_data = mysql_fetch_array(mysql_query("SELECT apply_num FROM recruit_research WHERE wr_id='$wr_id'"));
...
$query2 = "profile = '$profile',
							...
							password = '$pass'";
if($j==""){
    $sql = "INSERT recruit_copy_research SET apply_num='$apply_num',$query1 ,file_name='$file_name1', s_file_name='$s_file_name1'";
    $result = mysql_query($sql);
    $wr_ins_id = mysql_insert_id();
    $sql1 = "INSERT recruit_research SET parent = '$wr_ins_id',wr_datetime=now(),type_gubun='$type_gubun',$query2";
    $result1 = mysql_query($sql1);
    if(!$result1) error_log("recruit5/proc.php INSERT recruit_research 실패 (wr_id=$wr_ins_id): ".mysql_error());
    if($result && $result1){
    ...
    $sql = "UPDATE recruit_copy_research SET apply_num='$apply_num', $query1 ";

    $result = mysql_query($sql);
    $sql1 = "UPDATE recruit_research SET type_gubun='$type_gubun',$query2 WHERE parent='$wr_id'";
    $result1 = mysql_query($sql1);
    if(!$result1) error_log("recruit5/proc.php UPDATE recruit_research 실패 (wr_id=$wr_id): ".mysql_error());
    if($result && $result1){
