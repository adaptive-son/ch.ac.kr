            ...
            <div class="container-wrapper">
                ...
                <article>
                    <div class="contents" id="contents">
                        ...
                        //채용공고 확인
                        $check = mysql_num_rows(mysql_query("SELECT * FROM recruit_index_bi WHERE idx='$resume_num'"));

                        if ( $_SERVER["REMOTE_ADDR"] == "112.217.216.250" ) {
                            // 본사접근시, 진행중인 공고여부와 상관없이 최근 데이터 접근
                            if ( empty($resume_num) ) {
                                $sql = " select * from recruit_index_bi where idx = ( select max(idx) from recruit_index_bi ) ";
                                $rs = mysql_query($sql);
                        ...
                        $sql = "SELECT * FROM recruit_copy_bi WHERE kor_name='$wr_name' AND phone='$phone' AND resume_num = '$resume_num'";
                        $res = mysql_query($sql);
