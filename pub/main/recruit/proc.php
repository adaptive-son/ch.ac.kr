<?php
	// ê°œë³„ ?˜?´ì? ?¤í–‰ ë°©ì?. ?´ë‹?ë¬¸ì?´ì€ inc.constant.php? ?•?˜?œ??
	define("__AF__", TRUE);
	// adframe ?œ?Œë¦??˜?´ì? ?¤ì •.
	...
	if($file1_size>0&&$file1) {
		...
		if(!is_uploaded_file($file1)) Error("?•????ë°©ë??¼ë? ?…ë¡œ?œ ?´ì£¼?¸ìš”");
		...
		if($file1_size>0) {
			...
			if (!in_array(strtolower(end($temp1)), $allowed_ext)) { Error("?ˆ?©ë˜ì§€ ?Š?” ?Œ???•??…?ˆ??"); }
			...
			if(file_exists("./file_data/$id/".$temp1_name)) {
				...
				if(!move_uploaded_file($file1,"./file_data/$id/".$reg_date."/".$temp1_name)) Error("ÆÄÀÏ¾÷·Îµå°¡ ¿Ã¹Ù·Î µÇÁö ¾Ê¾Ò½À´Ï´Ù");
				$file_name1 = "/recruit/file_data/$id/".$reg_date."/".$temp1_name;
				@chmod($file_name1,0706);
				@chmod("./file_data/$id/".$reg_date,0707);
			} else {
				if(!move_uploaded_file($file1,"./file_data/$id/".$temp1_name)) Error("ÆÄÀÏ¾÷·Îµå°¡ ¿Ã¹Ù·Î µÇÁö ¾Ê¾Ò½À´Ï´Ù");
	...
	if($apply_major == "ê°„í˜¸í•™ê³¼"){
		$apply_num_1 = "ê°„í˜¸";
		$apply_num_etcIdx = "02";
	}elseif($apply_major=="?‘?…ì¹˜ë£Œê³?"){
		$apply_num_1 = "?‘ì¹˜";
		$apply_num_etcIdx = "";
	}else{
		$apply_num_1 = str_replace("ê³¼","",$apply_major);
		$apply_num_etcIdx = "";
	}
...
*/
	...
	if($j==""){
		...
		if($result){
			...
			echo "<script>alert('ì´ë ¥ì„œê°€ ì •ìƒì ìœ¼ë¡œ ì ‘ìˆ˜ë˜ì—ˆìŠµë‹ˆë‹¤');location.href='./'</script>";
		}else{
			echo "<script>alert('ì˜¤ë¥˜ê°€ ë°œìƒí•˜ì˜€ìŠµë‹ˆë‹¤');history.back();</script>";
		}
	}else if($j=="u"){
		...
		if($result){
			...
			echo "<script>alert('ì´ë ¥ì„œê°€ ì •ìƒì ìœ¼ë¡œ ìˆ˜ì •ë˜ì—ˆìŠµë‹ˆë‹¤');location.href='./resume.php?j=u&wr_id=$wr_id&pass=$pass_check'</script>";
		}else{
			echo "<script>alert('ì˜¤ë¥˜ê°€ ë°œìƒí•˜ì˜€ìŠµë‹ˆë‹¤');history.back();</script>";
		}
	}
