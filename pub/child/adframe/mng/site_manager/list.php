...
$sql = " select * from ".TABLE_SITE_MNG." where 1=1 ".$search_qry;
$rs = $adb->query($sql);
// 총 레코드 수
$numrows = $rs->numRows();
            ...
            <fieldset>
                <div class="board-search-area">
                    <form name="searchForm" method="POST" action="<?=$PHP_SELF?>" onsubmit="return searchSendit();">
                        ...
                        <div class="board-search-box">
                            <select name="search">
                                <?
                                $arr_searchSelect = array("site_id"=>"사이트ID", "site_name"=>"사이트명","site_domain"=>"사이트도메인");
                ...
                <div class="btns-area">
                    <div class="btns-right">
                        <a href="edit.php" class="btn-type01">
                            추가
                        </a>
                    </div>
                </div>
                ...
                <div class="board-list">
                    <table style="width: 100%" summary="이 표는 게시판에 대해 목록정보를 제공하는 표입니다.">
                        ...
                        <tr>
                            <?
                            $arr_title = array("번호", "사이트아이디", "사이트명", "도메인", "관리");
                        ...
                        <? if ( $numrows == 0 ) { ?>
                            ...
                            $sql .= " order by site_no asc limit ".$offset.", ".$LIMIT;
                            $pg_result = $adb->query($sql);

                            for ( $i = 0 ; $row = $pg_result->fetchRow() ; $i++ ) {
                                ?>
                                <tr>
                                    <td align="center">
                                        <?=$letter_no?>
                                    </td>
                                    <td align="center"><? echo $row[site_id]?></td>
                                    <td align="center"><a href="edit.php?j=u&site_no=<?= $row[site_no]?>"><? echo $row[site_name]?></a></td>
                                    <td align="center">
                                        <a href="<?=$row[site_domain]?>" target="_blank">
                                            <? echo $row[site_domain]?>
                                        </a
                                    </td>
                                    <td>
                                        <a href="edit.php?j=u&site_no=<?= $row[site_no]?>"><img src="../make_img/board/btn_modify.gif" align="absmiddle" alt="수정"></a>
                                        <a href="javascript:del('update.php?j=d&site_no=<?= $row[site_no]?>')"><img src="../make_img/board/btn_delete.gif" align="absmiddle" alt="삭제"></a>
                                    </td>
                                </tr>
                                <? $letter_no--; } } ?>
                        </tbody>
