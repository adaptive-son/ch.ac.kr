...
if($wr_id != ""){
    ...
    $school_addr = mysql_fetch_array(mysql_query("SELECT * FROM recruit_te_school_addr WHERE parent='$data1[wr_id]'"));
...
<table width="1000" class="table_box_recruit">
    ...
    <tr>
        <td colspan="8" class="sub_title">2.인적사항사항</td>
    </tr>
    <tr>
        <th>성별</th>
        <td colspan="3"><?=$data1[sex]?></td>
        <th>병역</th>
        <td colspan="3"><?=$data1[army]?></td>
    </tr>
    <?
	...
	<tr>
        ...
        <td><?=$data1[colleage1]?></td>
        <td><?=$school_addr[colleage1_addr]?></td>
        <td><?=$data1[cMajor1]?></td>
        <td><?=$data1[cDegree1]?><br /> (<?=$data1[cDegree1_date]?>)</td>
