...
<div class="board-area">
	...
	<div class="thumnbnail-list-wrapper01">
	<?
	...
	if($SecAdmin != 1 && $configBBS[auth_list] && @strpos(",".$configBBS[auth_list], $bbs_authgroup) == false){
		...
		$BBS_TitleLink2 = "<a href='".$PHP_SELF."?bbs=see&data=".$list_data."'><img src=\"$upImg\" width='126' height='105' id=\"photo\" onLoad=\"sizeModify(this);\" onClick=\"openImageWinCenter(this.src)\" alt=\"".$BBS_Title." 썸네일 이미지\" style=\"cursor:hand;\"></a>";
	...
	?>
		...
		<div class="thumnbnail-area01">
			<a href="<?=$PHP_SELF?>?site_id=<?=TREE_ID?>&TREE_NO=<?=$TREE_NO?>&DEPTH=<?=$DEPTH?>&bbs=see&data=<?=$list_data?>">
				<p class="date">
					<strong class="mmnn">
						<?=$writeday[1]?>. <?=$writeday[2]?>
					</strong>
					<span class="yy">
						<?=$writeday[0]?>
					</span>
				...
				<div class="thumnbnail-box01">
					...
					<p class="substance">
						...
						$bbs_row['content'] = strip_tags($bbs_row['content']);
