...
<div class="board-area">
    ...
    <form name="searchForm" action="<?=$PHP_SELF?>?TREE_NO=<?=$TREE_NO?>&DEPTH=<?=$DEPTH?>" method="post" onSubmit="return searchSendit();">
        <fieldset>
            <legend class="blind">
                게시판 목록 검색
            </legend>
            <div class="search-wrapper">
                <div class="search-area">
                    ...
                    <div class="search-box">
                        <input type="search" id="searchstring" name="searchstring" value="" title="검색어 입력">
		...
		<ul class="photo-list-wrapper">
			...
			//새글이미지
			/*
			if(BetweenPeriod($bbs_row[writeday],"1") > 0) $newImg = "<img src='/bbs/skin/$configBBS[board_skin]/images/icon_new.gif' border='0'>";
			else $newImg = "";
			*/

			//리스트 이미지
			if($bbs_row[up_file]>0){
                ...
                if(preg_match("/<img[^>]*src=[\"']?([^>\"']+)[\"']?[^>]*>/i", $bbs_row['content'], $match)){
                    ...
                    $upImg = "/_common/img/common/img_default.jpg";
                }
			}
			...
			*/
			...
			/*
			if($bbs_row[notice] == "Y") $list_no = "<img src='/bbs/skin/$configBBS[board_skin]/images/icon_notice.gif' alit='공지사항'>";
