...
$sql_menu .= " WHERE TREE_ID = '".TREE_ID."' AND a.MENU_ON = 'Y'  ORDER BY PARENT, ORDER_NO ";
...
for ( $i = 0 ; $row_menu = $rs_menu->fetchRow(DB_FETCHMODE_ASSOC); $i++ ) {
    ...
    switch ( $row_menu[ETC1] ) {
        ...
        case 'LINK': 
			...
			if($row_menu[LINK_TARGET]==" target = '_blank' title = '새창열림' "){
			...
			}else if($row_menu[CONTENTS] == '/contents/contents_view.php?site_id=main&TREE_NO=16183&DEPTH=4'){
				$row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]);
			}else if($row_menu[CONTENTS] == '/file/240905.pdf'){
				$row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]);
			}else{
				$row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]."&".$prm_forMenu);
			}
