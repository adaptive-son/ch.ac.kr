...
$sql_menu .= " WHERE TREE_ID = '".TREE_ID."' ORDER BY PARENT, ORDER_NO ";
...
for ( $i = 0 ; $row_menu = $rs_menu->fetchRow(DB_FETCHMODE_ASSOC); $i++ ) {
    ...
    // 새창열림
    if ( $row_menu[LINK_TARGET] == "1" ) $row_menu[LINK_TARGET] = " target = '_blank' title = '새창열림' ";
    else $row_menu[LINK_TARGET] = "";
    // 링크 주소 설정
    switch ( $row_menu[ETC1] ) {
        case 'MENU': $row_menu[LINK_URL] = "javascript:void(0);"; break;
        case 'LINK': if($row_menu[LINK_TARGET]==" target = '_blank' title = '새창열림' "){
            $row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]);
        }else if($row_menu[CONTENTS] == '/main_schedule/list.php' || $row_menu[CONTENTS] == '/contents/campusTel.php'){
            $row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]."?".$prm_forMenu);
        }else if($row_menu[CONTENTS] == '../contents/contents_view_en.php?site_id=main&TREE_NO=16183&DEPTH=4'){
            $row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]);
        }else{
            $row_menu[LINK_URL] = urldecode($row_menu[CONTENTS]."&".$prm_forMenu);
        }
        ...
        case 'TAB':
        case 'TABUPPER':
            $row_menu[LINK_URL] = "../contents/contents_view_en.php?".$prm_forMenu;
            break;
        case 'BOARD':
...
for ( $i = $DEPTH ; $i > 0 ; $i-- ) {
    ...
    ${"PAGENODE".$i} = ${"find_".$i."depth"}[$PARENTNO][TREE_NO];
