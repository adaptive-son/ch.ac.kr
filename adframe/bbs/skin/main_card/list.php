...
<div class="board-area">
	...
	$BBS_Title  = StringCut($bbs_row[title],0,$configBBS[board_titlecut],'UTF-8','...');
	 ...
	 if($bbs_row[up_file]>0 && $z_row['img_cnt']>0){
		...
		$file_data = sql_fetch("SELECT * FROM ".$configBBS['board_id']."_file WHERE up_file_idx='".$bbs_row['up_file_idx']."' ORDER BY idx ASC LIMIT 1");
		//print_R($file_data);
		if($file_data['file_text']){
			$alt = $file_data['file_text'];
		}else{
			$alt = $BBS_Title;
		}
	}else{
