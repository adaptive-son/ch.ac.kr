...
if ( $_SESSION["managerBoardKey"] ) {
    $__board_auth_row = DBarray("SELECT idx FROM ".TABLE_BOARD_MNG." WHERE board_key='".addslashes($_SESSION["managerBoardKey"])."'");
    if ( $__board_auth_row ) {
        require_menu_auth('board', $__board_auth_row['idx']);
    }
}
