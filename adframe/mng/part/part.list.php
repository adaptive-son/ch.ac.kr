...
// 검색
if ( $searchstring ) {
    $search_qry = " $search LIKE '%".$searchstring."%' ";
}
                ...
                <div class="board-search-area" style="display: inline-block; width: 95%;">
                    <div class="board-search-box" style="float: right;">
                        <select name="search" style="width: 80px;">
                            <?
                            $arr_selectSearch = array("name"=>"부서명");
                    ...
                    <div class="span12">
                        ...
                        <table class="table table-hover" style="width: 100%;">
                            ...
                            <thead>
                                <tr>
                                    <?
                                    $arr_tblTitle = array("번호", "부서명", "순서", "편집"); // "URL",
                            ...
                            <tbody>
                                ...
                                <?
                                    ...
                                    for ( $i = 0 ; $pg_row = $pg_result->fetchRow() ; $i++ ) {
                                        $s_level = "";
                                        $level = strlen($pg_row[p_num]) / 2 - 1;
                                ?>
                                <tr>
                                    <td>
                                        <input type="hidden" name="way_id[<?=$i?>]" value='<?=$pg_row[p_num]?>'>
                                        <?= $letter_no ?>
                                    </td>
                                    <td>
                                        <!--<a href="write.php?w=u&p_num=<?= $pg_row[p_num] ?>"><span><?= $pg_row[t_category1] ?></span></a>-->
										<?= $pg_row[name] ?>
                                    </td>
                                    <td>
										<?= $pg_row[sort] ?>
                                    ...
                                    <td>
                                        <a href="write.php?w=u&p_num=<?= $pg_row[p_num] ?>"><button class="btn btn-info">수정</button></a>
                                        <a href="javascript:del_data(<?= $pg_row[p_num] ?>);"><button class="btn btn-success">삭제</button></a>
                                    </td>
                                </tr>
                                <?
                                        $letter_no--;
