...
$sql = "select * from ".TABLE_PROFESSOR." WHERE del_yn='N' AND site_id ='$_SESSION[sel_site_id]' ".$search_qry;
$rs = $adb->query($sql);
// 총 레코드 수
$numrows = $rs->numRows();
...
<head>
    ...
    <SCRIPT>
        function del(url) {
            var ans = confirm("해당 교수를 삭제 하시겠습니까?");
            if ( ans == true ) {
                location.href = url;
            }
        }
            ...
            <fieldset>
                <div class="board-search-area">
                    <form name="searchForm" method="POST" action="<?=$PHP_SELF?>" onsubmit="return searchSendit();">
                        ...
                        <div class="board-search-box">
                            <select name="search">
                                <?
                                $arr_searchSelect = array("name"=>"교수명");
                ...
                <div class="btns-area">
                    <div class="btns-right">
                        <a href="form.php" class="btn-type01">
                            추가
                        </a>
                    </div>
                </div>
                ...
                <div class="board-list">
                    <table style="width: 100%" summary="이 표는 게시판에 대해 목록정보를 제공하는 표입니다.">
                        ...
                        <tr>
                            <?
                            $arr_title = array("번호", "교수구분", "성명", "직위", "연락처", "전자우편", "등록일", "관리");
                        ...
                        <? if ( $numrows == 0 ) { ?>
                            ...
                            $sql .= " order by etc1 asc, sort asc, idx asc limit ".$offset.", ".$LIMIT;
                            ...
                            for ( $i = 0 ; $pg_row = $pg_result->fetchRow() ; $i++ ) {
                                ...
                                <tr>
									...
									<td>
										<?php
											if ($pg_row[etc1]=="1") echo "전임교수";
											else if ($pg_row[etc1]=="2") echo "겸임교수";
											else if ($pg_row[etc1]=="3") echo "외래교수";
											else if ($pg_row[etc1]=="4") echo "조교";
                      else if ($pg_row[etc1]=="5") echo "계장";
										?>
									</td>
                                    <td><?=$pg_row[name]?></td>
                                    <td><?=$pg_row[position]?></td>
                                    <td><?=$pg_row[tel]?></td>
                                    <td><?=$pg_row[email]?></td>
                                    <td>
                                        <?=$pg_row[regi_date]?>
                                    ...
                                    <td>
                                        <a href="form.php?mode=u&idx=<?= $pg_row[idx]?>"><img src="../make_img/board/btn_modify.gif" align="absmiddle" alt="수정"></a>
                                        <a href="javascript:del('proc.php?mode=d&idx=<?=$pg_row[idx]?>')"><img src="../make_img/board/btn_delete.gif" align="absmiddle" alt="삭제"></a>
                                    </td>
                                </tr>
                                <? $letter_no--; } } ?>
                        </tbody>
