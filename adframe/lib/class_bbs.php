    ...
    function makebbs($bbs, $BoardKey, $Sub_No="0", $SecAdmin="0", $bbs_userqry="", $bbs_subqry="", $bbs_subcolumnqry="",$is_site_type="front") {
        ...
        if($bbs=="list") {
			...
			*/
			...
			$num_query = "SELECT idx FROM ".$configBBS["board_id"]." WHERE idx > 0 AND del_yn = 'N' ".$Code_Que;
			if ( $searchstring ) $num_query .= " AND $search LIKE '%$searchstring%' ";
			$numresults = DBquery($num_query);
			...
			else if ( $BoardKey=="2616") $add_orderSql = " CAST(etc_char4 AS UNSIGNED) ASC, ";
            else $add_orderSql = "";
           ...
           // }
			...
			// 쿼리 수정 : 관리자의 경우 공지사항 위 아래 어떻게 처리할지 정리되면 부분 수정 23.09.05 KDG
			/* 
			// 쿼리 추가 2023.11.08 By.Son
			// - 오래된 게시글의 경우, category, notice_start, notice_end 값이 null인 경우가 존재
			// - null 값도 확인하는 쿼리 추가 
			// - 해당 데이터값이 없는 경우, 카테고리 검색에서 나타나지 않을 가능성이 높음
			*/
			//if($BoardKey=="1010") {
			//if($is_site_type!="admin"){ // 관리자 별도의 아래 검색열이 필요하면 코드 수정 해야함. 23.09.05 KDG
			// 공지글이 아닌 게시글을 불러오는 쿼리의 조건절 
				$and_query = "AND ( (notice = 'N' OR REPLACE(notice, ' ', '') = '' )
								OR ( notice = 'Y' AND ( DATE_FORMAT(notice_start, '%Y-%m-%d') > DATE_FORMAT(NOW(), '%Y-%m-%d') OR DATE_FORMAT(notice_end, '%Y-%m-%d') < DATE_FORMAT(NOW(), '%Y-%m-%d') ) ) ) ";

				/*
				$and_query = "AND (
					-- (notice = 'Y' AND NOW() BETWEEN notice_start AND notice_end)
					-- OR
					(notice = 'N' AND (notice_start = '' OR notice_end = '' OR notice_start ='0000-00-00 00:00:00' OR notice_end='0000-00-00 00:00:00'))
				)";
				*/
			//}
            ...
            if($searchstring){
                $bbs_qry = "SELECT 
							...
							FROM (
                                ...
                                WHERE idx > 0 AND del_yn='N' ".$Code_Que." AND notice='Y' AND $search LIKE '%$searchstring%' 
									AND ( ( DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(notice_start, '%Y-%m-%d') AND DATE_FORMAT(NOW(), '%Y-%m-%d') <= DATE_FORMAT(notice_end, '%Y-%m-%d') )
										OR notice_start is null OR notice_end is null OR notice_start like '%0000%' OR notice_end like '%0000%' OR replace(notice_start, ' ', '') = '' OR replace(notice_end, ' ', '') = '' )
                                UNION ALL
                                SELECT A.*, 'N' as top_yn
                                FROM ".$configBBS[board_id]." A
                                WHERE idx > 0 AND del_yn='N' ".$Code_Que." AND $search LIKE '%$searchstring%' ".$and_query."
                                )TB
				...
				if($configBBS['board_key']=="2610"){
					$bbs_qry = "SELECT TB.*, (
									SELECT COUNT(idx)
									FROM ".$configBBS[board_id]."_comment
									WHERE bbs_idx=TB.idx AND del_yn='N') comment_cnt
									FROM
									(
									SELECT A.*, 'Y' AS top_yn
									FROM ".$configBBS[board_id]." A
									...
									AND (
										(NOW() BETWEEN notice_start AND notice_end)
										OR
										(notice_start ='' OR notice_end='' OR notice_start ='0000-00-00 00:00:00' OR notice_end='0000-00-00 00:00:00' OR notice_start is null OR notice_end is null )
									)
									UNION ALL
									SELECT A.*, 'N' AS top_yn
									FROM ".$configBBS[board_id]." A
				...
				*/
				...
				// 2024.01.31 By.Son. 쿼리 정리 
				// 검색어가 포함되지 않은 쿼리 정리 : Code가 '2610(대표사이트 - 공지사항)'인 게시판인 경우, 정렬순서(Order By)만 다른 내용이라 정리 
				// 공지글과 관련된 쿼리부분에서 누락데이터가 발생하는 것으로 추측하여 쿼리를 변경 
				// 확인결과 :: 골지글(notice='Y')인 게시글은 빠짐없이 가져오나, 공지글이 아닌 나머지 글을 제대로 가져오지 못함 
				// >>>> notice 값이 없는 경우나, notice = 'N'임에도 공지시간(notice_start, notice_end) 값을 검색하는 등의 내용이 확인되어 일부 쿼리를 수정함 
				$bbs_qry = "
					SELECT 
						TB.*, 
						( SELECT COUNT(idx) FROM ".$configBBS[board_id]."_comment WHERE bbs_idx=TB.idx AND del_yn='N') comment_cnt
					FROM (
						SELECT A.*, 'Y' AS top_yn
						FROM ".$configBBS[board_id]." A
						WHERE idx > 0 AND del_yn='N' ".$Code_Que." AND notice='Y'
							AND ( ( DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(notice_start, '%Y-%m-%d') AND DATE_FORMAT(NOW(), '%Y-%m-%d') <= DATE_FORMAT(notice_end, '%Y-%m-%d') )
								OR notice_start is null OR notice_end is null OR notice_start like '%0000%' OR notice_end like '%0000%' OR replace(notice_start, ' ', '') = '' OR replace(notice_end, ' ', '') = '' )
						UNION ALL
						SELECT A.*, 'N' AS top_yn
